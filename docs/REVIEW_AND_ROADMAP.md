...
- 解析、下载卡片和监控主链路的 Swing EDT 违规已在后续核心体验里程碑修复；固定像素布局、查询单线程吞吐和其他次要窗口的线程模型仍需继续治理。
...
仍需人工验收：真实 Windows/macOS GUI 连续操作、真实 B站解析与音频/视频下载、暂停/继续/失败重试、ffmpeg 合并和登录流程均为 `NOT VERIFIED`。固定尺寸布局、单线程下载地址查询的吞吐上限、系统密钥库、签名安装包及真实双平台发布验收仍在后续路线中。
